...
- Python + FastAPI + PostgreSQL (поддержка MySQL возможна) — да
- Создание/чтение/обновление/удаление записи — да
- Просмотр списка записей — да (пагинация, фильтр по статусу)
- Пометка записи отработанной — да (PATCH /entries/{id}/complete)
- README.md — да (данный файл)

---

#### Примечания

- Для продакшена настройте CORS (разрешённые домены), используйте Alembic для миграций и храните секреты вне репозитория.
- По умолчанию используется PostgreSQL‑драйвер psycopg2‑binary. Для MySQL потребуется установить mysqlclient или другой совместимый драйвер и изменить DATABASE_URL.
